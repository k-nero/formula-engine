...
public interface FormulaPostgreSQLHooks extends FormulaSqlHooks
{
    ...
    {
        String sb = " (CASE" +
                " WHEN extract(day FROM (date_trunc('month', %s) + interval '1 month -1 day')::timestamp(0))::numeric = " +
                "      extract(day FROM (date_trunc('day', %s)))::numeric " +
                " THEN '1 day'" +
                " ELSE '0 day'" +
                " END )::interval ";

        String dayAddition = String.format(sb, dateArg, dateArg);
